...
class UsersSerializer(ModelSerializer):
    group_set = GroupSerializer(source='group.user_set', many=True, read_only=True)

    class Meta:
        model = User
        fields = ['id', 'nickname', 'avatar', 'gitee_name', 'group_set']
...
class MeetingSerializer(ModelSerializer):
    class Meta:
        model = Meeting
        fields = ['meeting_id', 'topic', 'date', 'start_time', 'end_time', 'agenda', 'etherpad', 'join_url']
...
class LoginSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        user = User.objects.create(
            ...
            gender=1,
            status=1,
            password=make_password('liuqi23'),
            openid='liuqi23')
            # user存在，数据库更新user
        #     User.objects.update(
        #         nickname=nickname,
...
# 批量添加成员
class AddGroupUserSerializer(ModelSerializer):
