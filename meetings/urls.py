...
from meetings.views import LoginView, GroupView, GroupsView, MeetingView, MeetingsView, UsersExcludeView, UserView, \
    ...
    UsersIncludeView, UserGroupView, MeetingsInGroupView
...
urlpatterns = [
    ...
    path('meetings_weekly/', MeetingsWeeklyView.as_view()),  # 查询未来一周会议
    path('meetings_weekly_in_group/<int:pk>/', MeetingsInGroupView.as_view()),  # 查询该组未来一周的会议
    path('meetings_daily/', MeetingsDailyView.as_view()),   # 查询当日会议
    path('meetings/<int:mid>/', MeetingDelView.as_view()),  # 删除会议
    path('meetings/<int:pk>', MeetingView.as_view()),  # 查询单个会议
    path('users_exclude/<int:pk>/', UsersExcludeView.as_view()),  # 查询不在该组的所有成员
    path('users_include/<int:pk>/', UsersIncludeView.as_view()),  # 获取该SIG组的所有成员
    ...
    # path('groupuser_del/<int:pk>/', GroupUserDelView.as_view()),  # 批量删除SIG组成员
]
